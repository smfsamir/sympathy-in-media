...
def dateExtractor(datestr):
    ...
    # will only work for Mon DD, YYYY format so pretty cbc specific but can probably change the regex and generalize this function
    date_found = re.search('([A-Za-z]{3} \d{1,2}, \d{4})', published_on)
    if date_found:
...
async def cbc_extractor(url, person, event_date):
    schema = {"Person Name": pl.Utf8, "Event Date": pl.Date, "Publication Date": pl.Date, "Publisher": pl.Utf8, "URL": pl.Utf8, "Paragraph Index": pl.Int64, "Paragraph Text": pl.Utf8}
    ...
    async with async_playwright() as p:
        ...
        # todo
        # - need to deal with linked tweets as they are formatted weird and include link tags as text
        ...
        index = 0
        for p in paras:
            temp = pl.DataFrame({"Person Name": person, "Event Date": event_date, "Publication Date": published_on, "Publisher": publisher, "URL":url, "Paragraph Index": [index], "Paragraph Text": p})
            df = df.vstack(temp)
            index += 1
        df.rechunk()
